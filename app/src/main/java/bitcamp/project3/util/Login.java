...
public class Login {
  ...
  public static void authUser() {
    ...
    saveUser(user);
    System.out.printf("%s님 환영합니다.", user.getUserID());
  }

  private static void saveUser(User user) {
    String datName = user.getUserID();
    try (FileOutputStream fos = new FileOutputStream(datName+".dat");
  ...
  private static User loadUser(String userID) {
    try (FileInputStream fis = new FileInputStream(userID+".dat");
