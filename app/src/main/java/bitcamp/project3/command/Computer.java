...
//            String command = Prompt.input("반납하실 책의 이름을 입력해주세요.\n");
            ...
            while (iterator.hasNext()) {
                ...
                if (value.getTakesOutBook().getBookCode() == bookNo) {
                    iterator.remove();
                    found = true;
                    System.out.println("도서 " + value.getTakesOutBook().getBookTitle() + "(이)가 성공적으로 반납되었습니다.");
                    break;
                }
            }
            if (!found) {
    ...
    private void printBookCheck(List<TakeOutRecord> values, boolean isNeedNo) {
        ...
        String title = isNeedNo ? "| 책번호 | 대여한 책 이름                           |   대출일   |   반납일   |" : "| 대여한 책 이름                           |   대출일   |   반납일   |";
        ...
        for (TakeOutRecord value : values) {
            ...
            String isOverdue = returnDate.isBefore(today) ?
                    ansiRed + returnDate + ansiEnd :
